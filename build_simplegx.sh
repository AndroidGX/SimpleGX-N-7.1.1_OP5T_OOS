...
#
# Modified by AndroidGX for SimpleGX KERNEL needs (all credits goes to andip71, not me!)

#######################################
# Parameters to be configured manually
#######################################

SIMPLEGX_VERSION="OP5T-OOS-N_v2.0"
...
DEFCONFIG="simplegx_defconfig"
DEFCONFIG_VARIANT=""

KERNEL_NAME="SimpleGX-KERNEL"
...
SIMPLEGX_FILENAME="${KERNEL_NAME,,}-$SIMPLEGX_VERSION"
...
step0_copy_code()
{
	...
	sed "s/\`echo \$LINUX_COMPILE_BY | \$UTS_TRUNCATE\`/$KERNEL_NAME-$SIMPLEGX_VERSION/g" -i $BUILD_PATH/scripts/mkcompile_h
	sed "s/\`echo \$LINUX_COMPILE_HOST | \$UTS_TRUNCATE\`/AndroidGX/g" -i $BUILD_PATH/scripts/mkcompile_h
}
...
step3_compile()
{
	...
	if [ "y" == "$COMPILE_DTB" ]; then
		...
		chmod 777 simplegx_tools/$DTBTOOL
		simplegx_tools/$DTBTOOL $DTBTOOL_CMD -o $BUILD_PATH/$OUTPUT_FOLDER/arch/$ARCHITECTURE/boot/dt.img -s 2048 -p $BUILD_PATH/$OUTPUT_FOLDER/scripts/dtc/ $BUILD_PATH/$OUTPUT_FOLDER/arch/$ARCHITECTURE/boot/
	fi
...
step4_prepare_anykernel()
{
	...
	cd $REPACK_PATH
	cp -R $BUILD_PATH/simplegx_anykernel/* .
...

	{
		...
		if [ "$(ls -A $BUILD_PATH/simplegx_modules)" ]; then
			cp $BUILD_PATH/simplegx_modules/* $MODULES_PATH
			cd $MODULES_PATH
	...
	cd $REPACK_PATH
	KERNELNAME="Flashing $KERNEL_NAME $SIMPLEGX_VERSION"
	sed -i "s;###kernelname###;${KERNELNAME};" META-INF/com/google/android/update-binary;
	COPYRIGHT="(c) AndroidGX, $(date +%Y.%m.%d-%H:%M:%S)"
...
step5_create_anykernel_zip()
{
	...
	# create zip file
	cd $REPACK_PATH
	zip -r9 $SIMPLEGX_FILENAME.recovery.zip * -x $SIMPLEGX_FILENAME.recovery.zip

	# sign recovery zip if there are keys available
	if [ -f "$BUILD_PATH/simplegx_tools/testkey.x509.pem" ]; then
		echo -e ">>> signing recovery zip\n"
		java -jar $BUILD_PATH/simplegx_tools/signapk.jar -w $BUILD_PATH/simplegx_tools/testkey.x509.pem $BUILD_PATH/simplegx_tools/testkey.pk8 $SIMPLEGX_FILENAME.recovery.zip $SIMPLEGX_FILENAME.recovery.zip_signed
		cp $SIMPLEGX_FILENAME.recovery.zip_signed $SIMPLEGX_FILENAME.recovery.zip
		rm $SIMPLEGX_FILENAME.recovery.zip_signed
	fi

	md5sum $SIMPLEGX_FILENAME.recovery.zip > $SIMPLEGX_FILENAME.recovery.zip.md5

	# Creating additional files for load&flash
	echo -e ">>> create load&flash files\n"

	cp $SIMPLEGX_FILENAME.recovery.zip cm-kernel.zip
...
step8_transfer_kernel()
{
	...
	if [ ! -z "$SMB_SHARE_KERNEL" ]; then
		smbclient $SMB_SHARE_KERNEL -U $SMB_AUTH_KERNEL -c "mkdir $SMB_FOLDER_KERNEL\\$SIMPLEGX_VERSION"
		smbclient $SMB_SHARE_KERNEL -U $SMB_AUTH_KERNEL -c "put $REPACK_PATH/$SIMPLEGX_FILENAME.recovery.zip $SMB_FOLDER_KERNEL\\$SIMPLEGX_VERSION\\$SIMPLEGX_FILENAME.recovery.zip"
		smbclient $SMB_SHARE_KERNEL -U $SMB_AUTH_KERNEL -c "put $REPACK_PATH/$SIMPLEGX_FILENAME.recovery.zip.md5 $SMB_FOLDER_KERNEL\\$SIMPLEGX_VERSION\\$SIMPLEGX_FILENAME.recovery.zip.md5"
		smbclient $SMB_SHARE_KERNEL -U $SMB_AUTH_KERNEL -c "put $REPACK_PATH/checksum $SMB_FOLDER_KERNEL\\$SIMPLEGX_VERSION\\checksum"
		smbclient $SMB_SHARE_KERNEL -U $SMB_AUTH_KERNEL -c "put $REPACK_PATH/cm-kernel.zip $SMB_FOLDER_KERNEL\\$SIMPLEGX_VERSION\\cm-kernel.zip"
		return
	fi
	...
	if [ ! -z "$SSH_FTP_REMOTE" ]; then
		...
		mkdir -p ~/bbuild_transfer/$SIMPLEGX_VERSION

		cp $REPACK_PATH/$SIMPLEGX_FILENAME.recovery.zip ~/bbuild_transfer/$SIMPLEGX_VERSION
		cp $REPACK_PATH/$SIMPLEGX_FILENAME.recovery.zip.md5 ~/bbuild_transfer/$SIMPLEGX_VERSION
		cp $REPACK_PATH/checksum ~/bbuild_transfer/$SIMPLEGX_VERSION
		cp $REPACK_PATH/cm-kernel.zip ~/bbuild_transfer/$SIMPLEGX_VERSION
...
step9_send_finished_mail()
{
	...
	if [ -z "$FINISH_MAIL_TO" ]; then
		...
		cat $ROOT_PATH/compile.log | /usr/bin/mailx -s "Compilation for $KERNEL_NAME $SIMPLEGX_VERSION finished!!!" $FINISH_MAIL_TO
	fi
}
...
display_help()
{
	...
	echo "Parameters:"
	echo
	echo "  SimpleGX version: $SIMPLEGX_VERSION"
	echo "  SimpleGX date:    $SIMPLEGX_DATE"
	...
	echo "  Kernel Filename: $SIMPLEGX_FILENAME"
	echo
	echo "======================================================================"
}
